...
function DashboardHome() {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: (data) => {
      queryClient.setQueryData<DashboardData>(
        ["customer-dashboard"],
        (current) => (current ? { ...current, profile: data } : current),
      );
  ...
  const past = reservations.filter((item) =>
    isFinished(item.reservation_status),
  ).length;
            ...
            <Card>
              <CardContent className="p-6">
                ...
                <h2 className="mt-3 font-serif text-2xl">
                  {nextRental.reservation_number}
                </h2>
                <p className="mt-2 flex items-center gap-2 text-sm text-muted-foreground">
                  <CalendarDays className="h-4 w-4" />
                  {formatDate(nextRental.start_date)} –{" "}
                  {formatDate(nextRental.end_date)}
                </p>
            ...
            <Card className="border-dashed">
              <CardContent className="p-6">
                ...
                <p className="mt-2 text-sm leading-6 text-muted-foreground">
                  When you reserve a trailer it will show up here with its dates
                  and status.
                </p>
                <a
                  href="/app/book"
  ...
  return (
    <Card>
      <CardContent className="p-5">
        ...
        <p className="mt-2 font-serif text-3xl tabular-nums text-primary">
          {value}
        </p>
      </CardContent>
    </Card>
...
async function loadDashboard(): Promise<DashboardData> {
  ...
  const [profileResult, reservationsResult] = await Promise.all([
    ...
    supabase
      ...
      .select(
        "id,reservation_number,start_date,end_date,total,reservation_status,payment_status",
      )
