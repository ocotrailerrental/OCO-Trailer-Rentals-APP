...
import { BookingSearch, Location, ReserveSearch, Trailer, addDays, formatMoney, parseReserveSearch, rentalEstimate, rentalDays, safeInternalRedirect } from '@/lib/booking'
...
function ReserveRoute() { return <BlinkClientBoundary fallback={<AuthLoading label="Loading reservation details…" />}><ReservationForm /></BlinkClientBoundary> }
...
function ReservationForm() {
  ...
  useEffect(() => {
    if (dataQuery.data?.profile) {
      // eslint-disable-next-line react-hooks/set-state-in-effect -- synchronize form defaults after the profile query resolves
      setValues(current => ({ ...current, name: current.name || dataQuery.data.profile?.full_name || '', phone: current.phone || dataQuery.data.profile?.phone || '' }))
    }
